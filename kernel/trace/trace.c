...
/**
 * trace_user_buffer_destroy - free up allocated memory of a trace user buffer
 * @tinfo: The descriptor to free up
 *
 * Frees any data allocated in the trace info dsecriptor.
 */
void trace_user_buffer_destroy(struct trace_user_buf_info *tinfo)
{
	char *buf;
	int cpu;
...
static int trace_user_fault_buffer_enable(struct trace_user_buf_info *tinfo, size_t size)
{
	char *buf;
	int cpu;
...
static void user_buffer_free(struct trace_user_buf_info **tinfo)
{
	...
	trace_user_buffer_destroy(*tinfo);
	kfree(*tinfo);
	*tinfo = NULL;
}
...
static int user_buffer_init(struct trace_user_buf_info **tinfo, size_t size)
{
	...
	ret = trace_user_fault_buffer_enable(*tinfo, size);
	if (ret < 0 && alloc)
...
/**
 * trace_user_buffer_init - Allocated or reference a per CPU buffer
 ...
 */
int trace_user_buffer_init(struct trace_user_buf_info *tinfo, size_t size)
{
	int ret;
...
/**
 * trace_user_buffer_get - up the ref count for the user buffer
 ...
 */
int trace_user_buffer_get(struct trace_user_buf_info *tinfo)
{
	if (!tinfo)
		return -1;
...
/**
 * trace_user_buffer_put - dereference a per cpu trace buffer
 * @tinfo: The @tinfo that was passed to trace_user_buffer_get()
 *
 * Decrement the ref count of @tinfo.
 *
 * Returns the new refcount (negative on error).
 */
int trace_user_buffer_put(struct trace_user_buf_info *tinfo)
{
	guard(mutex)(&trace_user_buffer_mutex);
 ...
 * @tinfo: The @tinfo allocated by trace_user_buffer_get()
 ...
 *   greater than the size passed to trace_user_buffer_init().
 */
