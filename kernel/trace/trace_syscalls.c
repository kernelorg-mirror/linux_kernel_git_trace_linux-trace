...
static int syscall_fault_buffer_enable(void)
{
	...
	if (syscall_buffer) {
		trace_user_buffer_get(&syscall_buffer->buf);
		return 0;
	}
	...
	ret = trace_user_buffer_init(&sbuf->buf, SYSCALL_FAULT_BUF_SZ);
	if (ret < 0) {
		kfree(sbuf);
		return ret;
	}
...
static void rcu_free_syscall_buffer(struct rcu_head *rcu)
{
	...
	trace_user_buffer_destroy(&sbuf->buf);
	kfree(sbuf);
}
...
static void syscall_fault_buffer_disable(void)
{
	...
	if (trace_user_buffer_put(&sbuf->buf))
		return;
...
static void ftrace_syscall_enter(void *data, struct pt_regs *regs, long id)
{
	...
	if (mayfault) {
		...
		user_ptr = sys_fault_user(tr, sys_data, sbuf, args, user_sizes);
		/*
